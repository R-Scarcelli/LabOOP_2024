...
class PlayWithSmartphoneArrays {
    ...
    static int search(Smartphone[] array, String brand) {
        ...
        for (var elements : array) {
            if (elements.getBrand().equals(brand)) {
                occurrencies++;
            }
        }
        return occurrencies;
    ...
    static int search(Smartphone[] array, boolean hasNFC, boolean hasGPS) {
        int occurrencies = 0;
        for (var elements : array) {
            if (elements.hasNFC() == hasNFC && elements.hasGPS() == hasGPS) {
                occurrencies++;
            }
        }
        return occurrencies;
    ...
    static int search(Smartphone[] array, int nCPUs, int sdSize, boolean has3G) {
        int occurrencies = 0;
        for (var elements : array) {
            if (elements.getCPUs() == nCPUs && elements.getSdSize() == sdSize && elements.has3G() == has3G) {
                occurrencies++;
            }
        }
        return occurrencies;
